...
- Co się dzieje przy reorder z niepełną listą columnIds? → Dostarczone kolumny reorderowane, pominięte zachowują stary order
...
- **FR-001**: System MUSI dostarczyć encję BoardColumn (extends BaseEntity) z polami: name (string), order (int, default 0), boardId (FK → boards)
...
- **FR-007**: `reorder(boardId, columnIds[])` MUSI bulk-update order kolumn
...
- **BoardColumn**: Kolumna tablicy Kanban — name (string), order (int), boardId (FK). Relacja ManyToOne → Board, OneToMany → Task.
...
- **SC-005**: DELETE board kaskadowo usuwa jego kolumny
- **SC-006**: GET /boards/:id zwraca board z załadowanymi kolumnami (eager load)
- **SC-007**: Swagger UI wyświetla tag "columns" z endpointami
- **SC-008**: `npm test -w apps/api` przechodzi po implementacji
